...
namespace PosturreRecoginzer
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        //record templates;
        int[][] templates = new int[postureNumber][];
        #region definitions
        /// <summary>
        ...
        public MainWindow()
        {
            ...
            ReadDefinePosture("../../PostureData.txt");
        }
                ...
                using (DrawingContext dc = this.drawingGroup.Open())
                {
                    ...
                    foreach (Body body in this.bodies)
                    {
                        ...
                        if (body.IsTracked)
                        {
                            ...
                            //计算坐标
                            int[] encodeNum = CalculateEncodeNumber(body);
                            //foreach(int i in encodeNum)
                            //{
                            //    Console.Write(i + " ");
                            //}
                            //Console.WriteLine();
        ...
        {
            // 获取所有点坐标
            List<CameraSpacePoint> jointPointList = new List<CameraSpacePoint>();
            foreach (JointType jointType in Enum.GetValues(typeof(JointType)))
            {
                jointPointList.Add(body.Joints[jointType].Position);
            }
            ...
            // 坐标平移
            for (int index = 0; index < jointPointList.Count; index++)
            {
                CameraSpacePoint point = jointPointList[index];
                point.X -= spinebase.X;
                point.Y -= spinebase.Y;
            }
            ...
            for (int index = 0; index < jointPointList.Count; index++)
            {
                encode[index * 2] = encodeOnePoint(gridWidth, jointPointList[index].X);
                encode[index * 2 + 1] = encodeOnePoint(gridHeight, jointPointList[index].Y);
            }
            return encode;
        }
        ...
        //选择最佳匹配的姿势
        private int SelectPoster(int[] encodeNum)
        {
            int result = 0;
            double minDiff = Double.MaxValue;
            for (int i = 0; i < templates.Length; i++)
            {
                double diff = compare(encodeNum, templates[i]);
                if (diff < minDiff)
                {
                    minDiff = diff;
                    result = i;
        ...
        //比较两个编码的差值
        private double compare(int[] s1, int[] s2)
        {
            double diff = 0;
        ...
        private void ReadDefinePosture(string path)
        {
            ...
            {
                templates[i] = new int[50];
                String str = streamReader.ReadLine();
                str.Trim();
                String[] strs = str.Split(' ');
                for (int j = 0; j < 50; j++)
                {
                    templates[i][j] = Convert.ToInt32(strs[j]);
                }
            }
        }
    }
